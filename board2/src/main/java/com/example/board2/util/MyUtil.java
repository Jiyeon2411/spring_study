...
public class MyUtil {
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl) {
		...
		int numPerBlock = 5; // 이전 6 7 8 9 19 다음 이전과 다음 사이의 숫자를 몇개를 표시할지
		int currentPageSetup; // 이전 버튼에 들어갈 값
		int page;
		
		if(currentPage == 0 || totalPage == 0) return ""; //디이터가 없다
		...
		//currentPage가 (1~4) (5~9) (10~14) (15~19)..일때 currentPageSetup 5 10 15..
		...
		while(page <= totalPage && page <= (currentPageSetup + numPerBlock)) {
			if(page == currentPage) {
				//현재 내가 선택한 페이지라면
				sb.append("<font color=\"pink\">" + page + "</font>&nbsp;");
			} else {
				//현재 선택한 페이지가 아니면
